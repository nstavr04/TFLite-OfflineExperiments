...
class TransferLearningModel(tf.Module):
  ...
  # This is basically get weights and biases. For some reason if i use a different name the input tensor gets zeroed
  # bottleneck not used, we just need an input for some reason it cant be empty
  @tf.function(input_signature=[
    tf.TensorSpec([None, NUM_FEATURES], tf.float32)
  ])

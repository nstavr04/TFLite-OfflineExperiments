...
class TransferLearningModel(tf.Module):
  ...
  def __init__(self, learning_rate=0.001):
    ...
    self.head = tf.keras.Sequential([
            # flatten unncecessary probably because we reshape the bottleneck
            ...
            # we use softmax in the training so probably not needed here
            # layers.Dense(
  ...
  def load(self, feature):
    ...
    feature_extractor = tf.keras.Model(x, bottleneck)
    feature_extractor.compile(optimizer=tf.keras.optimizers.SGD(lr=0.001), loss='categorical_crossentropy')
    # reshaped_bottleneck = tf.reshape(bottleneck, (-1, self.num_features))
  ...
  def train(self, bottleneck, label):
    ...
    """
    reshaped_bottleneck = tf.reshape(bottleneck, (-1, 7, 7, 1280))
    with tf.GradientTape() as tape:
        logits = self.head(reshaped_bottleneck)
    ...
    result = {'loss': loss}
    for var, grad in zip(self.head.trainable_variables, gradients):
